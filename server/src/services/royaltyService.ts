...
const handleRoyalty = async ({ address, amount, canvasAddress }) => {
  try {
    ...
    if (royalty) {
      ...
      if (userRoyalty) {
        console.log(`Royalty for user ${address} on canvas ${canvasAddress} exists. Updating amount.`);
        userRoyalty.amount += amount;
      } else {
        console.log(`Royalty for user ${address} on canvas ${canvasAddress} not found. Adding new entry.`);
        royalty.royalties.push({ address, amount });
      }

      await royalty.save(); // Save the updated document
      console.log(`Royalty for user ${address} on canvas ${canvasAddress} updated/added successfully.`);
      return;
    }

    // If no royalty document exists for the canvas, create a new one
    console.log(`Royalty document for canvas ${canvasAddress} not found. Creating new document.`);

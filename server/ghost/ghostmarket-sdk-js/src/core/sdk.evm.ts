...
export class GhostMarketSDK {
  ...
  constructor(
    ...
    this._chainFullName =
      ChainFullName[this._chainName as keyof typeof ChainFullName];
    this._chainCurrency =
      ChainCurrency[this._chainName as keyof typeof ChainCurrency];
    this._chainId =
      ChainId[this._chainName.toLowerCase() as keyof typeof ChainId];
      ...
      try {
        ...
        if (
          ...
          if (
            ...
            BigNumber.from(items[i].quotePrice).lt(
        ...
        if (items[i].type !== 1 && items[i].quoteContract !== "0x") {
          ...
          const hasApprovedEnough =
            BigNumber.from(amountApproved) >
            BigNumber.from(items[i].quotePrice);
        ...
        const supportsERC721 = await this._supportsERC721(
          items[i].baseContract
        );
        const supportsERC155 = await this._supportsERC1155(
          items[i].baseContract
        );
        ...
        if (items[i].type === 1 && supportsERC721) {
          const owner = await this._ownerOf(
            items[i].baseContract,
            items[i].baseTokenId!
          );
  ...
  public async bulkCancelOrders(
    ...
    try {
      const data =
        await ExchangeV2CoreContractInstance.methods.bulkCancelOrders(
          ordersArray
        );
  ...
  public async setRoyaltiesForContract(
    ...
    try {
      const data =
        await RoyaltiesRegistryContractInstance.methods.setRoyaltiesByToken(
          contractAddress,
          contractRoyalties
        );
      return this.sendMethod(
  ...
  public async wrapToken(
    ...
    if (isFromNativeToWrap) {
      ...
      try {
        const data = await WrappedTokenContractInstance.methods.withdraw(
          amount
        );
        return this.sendMethod(
          data,
  ...
  public async approveToken(
    ...
    try {
      const data = await ContractInstance.methods.approve(
        proxyContractAddress,
        MAX_UINT_256
      );
      return this.sendMethod(data, txObject.from, contractAddress, undefined);
  ...
  public async transferERC20(
    ...
    try {
      const data = await ContractInstance.methods.transfer(
        destinationAddress,
        amount
      );
      return this.sendMethod(data, txObject.from, contractAddress, undefined);
  ...
  public async burnERC1155(
    ...
    try {
      const data = await ContractInstance.methods.burn(
        txObject.from,
        tokenId,
        amount
      );
  ...
  public async checkTokenBalance(
    ...
    try {
      const data = await ERC20ContractInstance.methods.balanceOf(
        accountAddress
      );
      return await this.callMethod(data, accountAddress);
    } catch (e) {
  ...
  public async checkIncentives(accountAddress: string): Promise<any> {
    ...
    try {
      const data = await IncentivesContractInstance.methods.incentives(
        accountAddress
      );
  ...
  public async checkLPStakes(accountAddress: string): Promise<any> {
    ...
    try {
      const data = await LPStakingContractInstance.methods.userInfo(
        accountAddress
      );
      return this.callMethod(data, accountAddress);
    } catch (e) {
  ...
  public async checkLPRewards(accountAddress: string): Promise<any> {
    ...
    try {
      const data =
        await LPStakingContractInstance.methods.calculatePendingRewards(
          accountAddress
        );
      return this.callMethod(data, accountAddress);
    } catch (e) {
  ...
  public async stakeLPTokens(
    ...
    if (isStaking) {
      ...
      if (diff.gt(BigNumber.from(0))) {
        throw new Error(
          `Not enough balance to stake LP tokens, missing: ${BigNumber.from(
            diff
          )}`
        );
      }
  ...
  private _getLPTokenContractAddress(chainName: string): string {
    return AddressesByChain[chainName as keyof typeof AddressesByChain]!
      .LP_TOKEN!;
  }
